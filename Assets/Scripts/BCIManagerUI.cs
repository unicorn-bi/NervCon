...
{
    public class BCIManagerUI : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            //flash controller
            _flashController = gameObject.GetComponent<FlashController>();
        ...
        private void OnClassifierCalculationFailed(object sender, Exception e)
        {
            _calculatingClassifier = false;
            _classifierCalculated = false;
            _classifierCalculationFailed = true;

            Debug.Log(string.Format("Could not calculate classifier. {0}", e.Message));
        }
        ...
        void Update()
        {
            ...
            if (_startFlashing && _currentState == States.Acquiring)
            {
                ...
                {
                    if (_currentMode == ERPPipeline.Mode.Training)
                        _flashController.StartFlashing(FlashController.Mode.Training);
                    else
                        _flashController.StartFlashing(FlashController.Mode.Application);
                    _startFlashing = false;
                    _sw.Stop();
                }
            }
